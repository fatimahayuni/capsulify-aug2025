...
export const updateUserDetails = async (
	...
	try {
		...
		Object.entries(occasions).map(async ([key, value]) => {
			const occasionId = MONTHLY_OCCASIONS.find(
				(item) => item.key === key
			)?.id

			const insertOccassionQuery = `INSERT INTO user_monthly_occasions (user_id, occasions_id, occurence_count) VALUES ($1, $2, $3)`
			const insertOccassionResult = await client.query(
				insertOccassionQuery,
				[dbUserId, occasionId, value]
			)
